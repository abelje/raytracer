...
// ../cmake-build-debug/test/test-bvh-chart.exe input-spheres-1000 > bvh-runtime.csv
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // std::cout << "Total parsed objects: " << parser.get_world().objects.size() << std::endl;
        for (int count = 10; count <= 1000; count += 50) {
            ...
            render(bvh, camera, depth, static_cast<double>(samples)/num_threads, num_threads, pixels, false);
...
//     std::cout << "\rTest BVH Program: " << std::setw(3) << percentage << "% ";
